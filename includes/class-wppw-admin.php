...
class WebPagesPW_Admin {
	...
	public function render_settings_page(): void {
		...
		<div class="wrap wppw-settings-wrap">
			...
			<!-- ══════════════════════════════════════
			     SECCIÓN 5 · Login links (premium)
			     ══════════════════════════════════════ -->
			<?php $this->render_login_links_section( $is_premium, $locked ); ?>
	...
	private function render_login_links_section( bool $is_premium, string $locked ): void {
		$login_links = new WebPagesPW_LoginLinks();
		$users       = $login_links->get_users();
		$links       = $login_links->get_links();
		?>
		<div class="wppw-section<?php echo esc_attr( $locked ); ?>">
			<?php $this->premium_badge( $is_premium ); ?>
			<h2><?php esc_html_e( 'Login links', 'wppw' ); ?></h2>
			<p class="description"><?php esc_html_e( 'Genera enlaces que permiten a un usuario de WordPress iniciar sesión automáticamente sin introducir su contraseña. Útiles para onboarding, soporte o demos.', 'wppw' ); ?></p>

			<?php if ( isset( $_GET['wppw_login_created'] ) ) : ?>
				<div class="notice notice-success inline"><p><?php esc_html_e( 'Login link generado correctamente.', 'wppw' ); ?></p></div>
			<?php elseif ( isset( $_GET['wppw_login_error'] ) ) : ?>
				<div class="notice notice-error inline"><p><?php esc_html_e( 'El usuario seleccionado no existe.', 'wppw' ); ?></p></div>
			<?php endif; ?>

			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				<input type="hidden" name="action" value="wppw_login_create" />
				<?php wp_nonce_field( 'wppw_login_create' ); ?>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><label for="wppw_login_user"><?php esc_html_e( 'Usuario', 'wppw' ); ?></label></th>
						<td>
							<select id="wppw_login_user" name="user_id" <?php disabled( ! $is_premium ); ?>>
							...
							<?php if ( empty( $users ) ) : ?>
								<p class="description"><?php esc_html_e( 'No hay usuarios registrados.', 'wppw' ); ?></p>
							<?php endif; ?>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="wppw_login_label"><?php esc_html_e( 'Etiqueta (opcional)', 'wppw' ); ?></label></th>
						<td>
							<input type="text" id="wppw_login_label" name="label" class="regular-text"
								placeholder="<?php esc_attr_e( 'Ej. Acceso soporte mayo', 'wppw' ); ?>"
								<?php disabled( ! $is_premium ); ?> />
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="wppw_login_redirect"><?php esc_html_e( 'Redirigir a (opcional)', 'wppw' ); ?></label></th>
						<td>
							<input type="url" id="wppw_login_redirect" name="redirect_to" class="regular-text"
								placeholder="<?php echo esc_attr( home_url( '/' ) ); ?>"
								<?php disabled( ! $is_premium ); ?> />
							<p class="description"><?php esc_html_e( 'URL a la que se redirige al usuario tras el login. Por defecto: portada del sitio.', 'wppw' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="wppw_login_expires"><?php esc_html_e( 'Caducidad', 'wppw' ); ?></label></th>
						<td>
							<select id="wppw_login_expires" name="expires_in" <?php disabled( ! $is_premium ); ?>>
								<option value="3600"><?php esc_html_e( '1 hora', 'wppw' ); ?></option>
								<option value="86400" selected><?php esc_html_e( '1 día', 'wppw' ); ?></option>
								<option value="604800"><?php esc_html_e( '7 días', 'wppw' ); ?></option>
								<option value="2592000"><?php esc_html_e( '30 días', 'wppw' ); ?></option>
								<option value="0"><?php esc_html_e( 'Nunca', 'wppw' ); ?></option>
							</select>
						</td>
					...
					<tr>
						<th scope="row"><label for="wppw_login_uses"><?php esc_html_e( 'Usos máximos', 'wppw' ); ?></label></th>
						<td>
							<select id="wppw_login_uses" name="max_uses" <?php disabled( ! $is_premium ); ?>>
								<option value="1" selected><?php esc_html_e( '1 uso', 'wppw' ); ?></option>
								<option value="5"><?php esc_html_e( '5 usos', 'wppw' ); ?></option>
								<option value="10"><?php esc_html_e( '10 usos', 'wppw' ); ?></option>
								<option value="0"><?php esc_html_e( 'Ilimitado', 'wppw' ); ?></option>
							</select>
				...
				<?php if ( $is_premium ) : ?>
					<?php submit_button( __( 'Generar login link', 'wppw' ) ); ?>
				<?php else : ?>
					<p><a href="<?php echo esc_url( wppw_get_upgrade_url() ); ?>" class="button wppw-upgrade-btn">&#11088; <?php esc_html_e( 'Actualizar a Premium', 'wppw' ); ?></a></p>
				<?php endif; ?>
			</form>
			...
			<?php if ( $is_premium && ! empty( $links ) ) : ?>
				<h3><?php esc_html_e( 'Login links existentes', 'wppw' ); ?></h3>
				<table class="widefat striped wppw-magic-table">
					<thead>
						<tr>
							<th><?php esc_html_e( 'Usuario', 'wppw' ); ?></th>
							<th><?php esc_html_e( 'Etiqueta', 'wppw' ); ?></th>
							<th><?php esc_html_e( 'Enlace', 'wppw' ); ?></th>
							<th><?php esc_html_e( 'Caduca', 'wppw' ); ?></th>
							<th><?php esc_html_e( 'Usos', 'wppw' ); ?></th>
					...
					<tbody>
						<?php foreach ( $links as $token => $link ) :
							$u           = get_user_by( 'id', $link['user_id'] );
							$user_label  = $u ? $u->display_name . ' (' . $u->user_email . ')' : '#' . $link['user_id'];
							$link_url    = $login_links->get_link_url( $token );
							$expired     = $login_links->is_expired( $link );
							$exhausted   = $login_links->is_exhausted( $link );
							$uses_label  = $link['uses'] . ( $link['max_uses'] ? ' / ' . $link['max_uses'] : '' );
							?>
							<tr>
								...
								<td>
									<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="display:inline;">
										<input type="hidden" name="action" value="wppw_login_revoke" />
										<input type="hidden" name="token" value="<?php echo esc_attr( $token ); ?>" />
										<?php wp_nonce_field( 'wppw_login_revoke' ); ?>
										<button type="submit" class="button-link-delete"
											onclick="return confirm('<?php echo esc_js( __( '¿Revocar este login link? Dejará de funcionar inmediatamente.', 'wppw' ) ); ?>');">
											<?php esc_html_e( 'Revocar', 'wppw' ); ?>
										</button>
									</form>
								</td>
